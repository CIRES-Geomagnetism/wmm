...
def get_test_val(path= "WMM2025_FINAL_TEST_VALUES_HIGHPREC.txt"):
    lat = []
    lon = []
    alt = []
    time = []
    ...
    with open(path, 'r') as infile:
        for line in infile:
            ...
            test_X.append( np.float64(values[7]))
            test_H.append( np.float64(values[6]))
            test_dec.append(np.float64(values[4]))
            test_ydot.append( np.float64(values[15]))
